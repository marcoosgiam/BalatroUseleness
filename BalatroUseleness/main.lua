...
local items = {}
local items_directory = NFS.getDirectoryItems(mod_path .. item)
for i,file in ipairs(items_directory) do
    local f,err = SMODS.load_file(item.."/"..file)
    if err then
        print("couldn't load item:",file)
        --error(err)
    else
...
math.factorial = function(fact)
    local x=math.pi*2
    local a = math.exp(1)
    return math.sqrt(x * fact) * (fact / a) ^ fact 
end
SMODS.BalatroUtilities = {}
...
SMODS.read_function_as_string = function(f)--reads target function as a string being able to modify it and load it again later on.
    ...
    if func_source ~= nil then
        ...
        -- Logic insert begins
        local func_upvalues = SMODS.SourcedFunction:get_func_upvalues(f)
        local i = 1
        --[[while true do
            local name, val = debug.getupvalue(f, i)
            if not name then break end
            func_upvalues[name] = val
            i = i + 1
        end]]
        -- Logic insert ends

        return func_source,func_variables,func_upvalues
    end
    ...
    --print("source before any clean up:",function_source)
    --print("function line defined:",line_defined)
    local function clean_function_source(src)
        src = src:gsub("^=%[SMODS%s*", ""):gsub("%]$", "")
        src = src:gsub('%s(")', '/%1')
        src = src:gsub('"', "")
        return src
    end
    function_source="/"..clean_function_source(function_source)
    ...
    while true do
        local name, val = debug.getupvalue(f, i)
        if name then --[[print("found upvalue:",name)]] table.insert(func_upvalues, {name=name,val=val,index=i,}) end
        i = i + 1
        if i>=line_defined-last_line_defined then break end
    end
    --print("upvalues:",unpack(func_upvalues))
    return func_code, func_variables, func_upvalues
end
function extract_params(func_line)
    local param_str = func_line:match("function%s*%b()")
    if not param_str then
        param_str = func_line:match("return%s*function%s*%b()")
    end

    local params = {}
    if param_str then
        param_str = param_str:sub(2, -2) -- trim surrounding ()
...
local o_loadstring = loadstring -- Store the original loadstring
local o_print = print
local o_unpack = _G.unpack
local o_type = type
local o_smods = SMODS
_G.o_loadstring = function (...)
    --[[print('original loadstring called?')
    print("args:",...)]]
    local result = o_loadstring(...)
    --print('result:',result)
    return result
end
_G.loadstring = function(text, chunkName)
    --print("called loadstring with code:",text)
    local result = o_loadstring(text,chunkName)
    
    local function extract_params(func_str)
        local param_str = func_str:match("function%s*%((.-)%)")
        if not param_str then
            param_str = func_str:match("return%s+function%s*%((.-)%)")
        end
    ...
    -- Extracts the function *body* from a string
    local function extract_body(func_str)
        local body = func_str:match("function%s*%b()%s*(.-)%s*end$")
        if not body then
            body = func_str:match("return%s+function%s*%b()%s*(.-)%s*end$")
        end
        if body then
            body = body:gsub("^%s*", ""):gsub("%s*$", "") -- trim
        end
        return body
    end
    local function wrap_result(result, text)
        ...
        return function(self,...)
            ...
            if t_result then
            return_value = t_result(unpack(args))
            end
            if type(return_value) == "function" then
                return_value = wrap_result(return_value, text)
                local code=extract_body(text) or text
                o_print('new code:',code)
                SMODS.SourcedFunction:add_source(return_value, code,extract_params(text))
    ...
    local body=extract_body(text) or text
    local params = {}
    if body ~= text and body ~= nil then
        params=extract_params(text)
    end
    --print("set source:",body)
    local f = wrap_result(result,text)
    SMODS.SourcedFunction:add_source(f, body, params)
    return f
end
function STR_UNPACK(str)
    local func, err = loadstring(str)
    if not func then
        error("Error loading string: " .. tostring(err))
        return {}
    end
    ...
    if success then
        ...
        error("Error executing loaded function: " .. tostring(result))
        return {}
    end
end
...
SMODS.increase_object_value_relative = function (object, operation, val, include, exclude, percentage)
    val=val*(percentage*100)
    SMODS.increase_object_value(object,operation,val,include,exclude)
end
...
load_classes()
local card_edits = get_items("card_edits")
local function load_card_edits()
    local mod_count = SMODS.mod_list
    local function load()
        for i,v in ipairs(card_edits) do
            SMODS.BalatroUtilities:apply_card_edit(v)
        end
    end
    G.E_MANAGER:add_event(Event({
        func = function ()
