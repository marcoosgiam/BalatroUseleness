...
if self.ability.extra and self.area == G.jokers or self.ability.extra and self.area == G.consumeables then
--print('it has ability.extra')
local extra = self.ability.extra
if extra == nil then extra = self.effect.extra end
if type(extra) == "number" then extra = nil --[[print("set extra to nil for some reason")]] end
extra = extra or {}
...
for i,v in pairs(card_timers) do
    ...
    if active then
    --print("updating card timer id:",id)
